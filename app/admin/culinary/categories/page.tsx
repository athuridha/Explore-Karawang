...
export default function CulinaryCategoriesPage() {
  ...
  const loadAll = async () => {
    setLoading(true)
    setError(null)
    const res = await getCategories('culinary')
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    const res = await addCategory({ name: newName.trim(), type: 'culinary' })
    if (res.success && res.data) {
    ...
    <AdminLayout>
      <div className="max-w-5xl mx-auto">
        <div className="mb-8">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">Restaurant Categories</h1>
            <p className="text-gray-600">Create and organize restaurant categories</p>
          </div>
        </div>
            ...
            <form onSubmit={handleAdd} className="flex flex-col sm:flex-row gap-4 items-start sm:items-end">
              <div className="flex-1 w-full">
                ...
                <Input
                  ...
                  placeholder="e.g. Seafood, Traditional, Fusion"
                  disabled={adding}
                />
              </div>
